...
function parseCsv(text: string): ThermalRange[] {
  ...
  for (const rawLine of text.split(/\r?\n/)) {
    ...
    if (kind !== 'cpu' && kind !== 'gpu') continue;
    const match = parts[1]?.trim() ?? '';
    const minC = Number(parts[2]);
    const maxC = Number(parts[3]);
    if (!Number.isFinite(minC) || !Number.isFinite(maxC) || maxC <= minC) continue;
...
const DEFAULTS: Record<ThermalKind, ThermalRange> = {
  cpu: {
    kind: 'cpu',
    match: '*',
    minC: 0,
...
export function resolveThermalRange(
  kind: ThermalKind,
...
export function formatThermalRange(range: ThermalRange): string {
