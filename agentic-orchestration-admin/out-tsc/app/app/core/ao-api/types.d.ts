export type AoStatus = 'healthy' | 'degraded' | 'failed' | 'unset' | 'running' | 'info' | 'not_deployed';
export type AoSource = 'process-env' | 'process' | 'env.jetson' | 'tracked' | 'tool_env' | 'web_env' | 'example' | 'k8s-secret' | 'yaml-catalog' | 'runtime' | 'default' | 'unset' | 'unknown';
export type AoApplyTier = 'live' | 'next-run' | 'next_run' | 'next-session' | 'next_session' | 'restart-web' | 'restart-engine' | 'redeploy' | 'restart';
...
export type ControlTargetKind = 'k8s-deployment' | 'k8s-stack' | 'host-service' | 'host-reboot';
