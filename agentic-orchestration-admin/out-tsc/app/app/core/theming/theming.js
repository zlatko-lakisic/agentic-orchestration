...
let Theming = class Theming {
    ...
    // State
    prefersDarkMode = this.media.match('(prefers-color-scheme: dark)');
    colors = signal({
        primary: this.themeConfig.primary,
        error: this.themeConfig.error,
    });
    scheme = signal(this.localStorage.getItem('scheme') || this.themeConfig.scheme);
    theme = computed(() => this.generateTheme(this.colors()));
    isDark = computed(() => this.scheme() === 'dark' ||
        (this.scheme() === 'system' && this.prefersDarkMode));
    isLight = computed(() => !this.isDark());
    // DOM
    rootEl = this.document.documentElement;
    themeStyleEl = this.document.createElement('style');
    constructor() {
