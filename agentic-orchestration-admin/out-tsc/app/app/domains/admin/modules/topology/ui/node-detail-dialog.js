...
let NodeDetailDialog = class NodeDetailDialog {
    ...
    healthSeries = signal([]);
    trafficRate = signal([]);
    trafficLatency = signal([]);
    trafficActive = signal(false);
    trafficInstrumented = signal(false);
    ...
    ownerLabel(d) {
        ...
        const apps = d.ownedByApps?.length
            ? d.ownedByApps
    ...
    k8sPods(d) {
        const fromMembers = d.members?.pods;
        const pods = fromMembers?.length
            ? fromMembers
    ...
    healthChartSeries() {
        return [{ name: 'latency ms', data: this.healthSeries() }];
    }
    trafficRateSeries() {
        return [{ name: 'rate', data: this.trafficRate() }];
    }
    trafficLatencySeries() {
        return [
            {
                name: 'p95 ms',
    ...
    applyWatch(ev) {
        ...
        if (latest?.status)
            this.liveStatus.set(String(latest.status));
        const health = ev['health'] || [];
        if (health.length)
            this.healthSeries.set(health);
        ...
        this.trafficInstrumented.set(Boolean(ev['instrumented']) && (rate.length > 0 || lat.length > 0));
    }
    teardown() {
