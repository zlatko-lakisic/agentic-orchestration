...
const WEB_API_KIND_ORDER = {
    'ao-web': 0,
    'ao-chat': 1,
    openclaw: 2,
    'web-api-client': 3,
};
function sortWebApiIds(ids, kindById) {
    return [...ids].sort((a, b) => {
...
function slotFor(node, appLaneById, expandedAppId) {
...
function filterApplicationAccordion(nodes, expandedAppId) {
    return nodes.filter((n) => {
        if (n.band !== 'application' || !n.appId)
            return true;
        if (n.kind === 'app' || isBypassAppKind(n.kind))
            return true;
        if (APP_CHILD_LANE[n.kind] == null)
            return true;
...
function pathHitsObstacles(pts, obstacles) {
    for (let i = 0; i < pts.length - 1; i++) {
        for (const o of obstacles) {
            if (segHitsRect(pts[i], pts[i + 1], o))
                return true;
        }
    }
    return false;
}
function isHoriz(a, b) {
    return Math.abs(a.y - b.y) < 0.5;
}
function isVert(a, b) {
    return Math.abs(a.x - b.x) < 0.5;
}
function pathToSegs(pts) {
    const segs = [];
    for (let i = 0; i < pts.length - 1; i++) {
        const a = pts[i];
        const b = pts[i + 1];
...
function pathOverlapsReserved(pts, reserved, sep = EDGE_SEP) {
    ...
    for (const s of segs) {
        for (const r of reserved) {
            if (segsCollinearOverlap(s, r, sep))
                return true;
        }
    }
    return false;
...
export function pathsCollinearOverlap(a, b, sep = EDGE_SEP) {
...
export function pathPoints(pathD) {
    const pts = [];
    const re = /([ML])\s*(-?\d+(?:\.\d+)?)\s+(-?\d+(?:\.\d+)?)|Q\s*-?\d+(?:\.\d+)?\s+-?\d+(?:\.\d+)?\s+(-?\d+(?:\.\d+)?)\s+(-?\d+(?:\.\d+)?)/gi;
    let m;
 ...
 */
export function edgePathHitsOtherNodes(pathD, fromId, toId, allNodes, clearance = CLEARANCE) {
    ...
    const mid = pts.length >= 4 ? pts.slice(1, pts.length - 1) : pts;
...
function pathInCanvas(pts) {
    for (const p of pts) {
        if (p.x < MIN_ROUTE_X - 0.5 || p.y < MIN_ROUTE_Y - 0.5)
            return false;
    }
    return true;
}
function pickClearPath(candidates, obstacles, reserved = []) {
    let best = null;
...
function simplifyOrtho(pts) {
    ...
    for (let i = 1; i < pts.length - 1; i++) {
        ...
        const cur = pts[i];
        const next = pts[i + 1];
        const colinear = (Math.abs(prev.x - cur.x) < 0.5 && Math.abs(cur.x - next.x) < 0.5) ||
...
function toPathD(pts) {
    ...
    for (let i = 1; i < clean.length - 1; i++) {
        ...
        parts.push(`L ${round(enter.x)} ${round(enter.y)} Q ${round(cur.x)} ${round(cur.y)} ${round(leave.x)} ${round(leave.y)}`);
    }
...
function sideCenter(node, side, along = 0) {
...
function stubOut(node, side, stub = PORT_STUB, along = 0) {
...
function chooseSides(from, to, kind, allNodes = []) {
    if (kind === 'bypass') {
    ...
    let fromSide = dx >= 0 ? 'right' : 'left';
    let toSide = dx >= 0 ? 'left' : 'right';
 ...
 */
export function routeEdgeOrthogonalPoints(from, to, kind, allNodes, canvasWidth, opts) {
    ...
    const farLeft = Math.max(MIN_ROUTE_X, Math.min(leftX, ...obstacles.map((o) => o.x), a.x, b.x) - 28);
    const farRight = Math.max(rightX, ...obstacles.map((o) => o.x + o.w), a.x, b.x) + 28;
    const farTop = Math.max(MIN_ROUTE_Y, Math.min(gutterAbove, ...obstacles.map((o) => o.y), a.y, b.y) - 28);
    const farBottom = Math.max(gutterBelow, ...obstacles.map((o) => o.y + o.h), a.y, b.y) + 28;
...
export function routeEdgeOrthogonal(from, to, kind, allNodes, canvasWidth, opts) {
    return toPathD(routeEdgeOrthogonalPoints(from, to, kind, allNodes, canvasWidth, opts));
}
/** Parse path endpoints for tests. */
export function pathEndpoints(pathD) {
...
export function isSideCenter(node, p, tol = 1.5) {
    const onTop = Math.abs(p.y - node.y) <= tol &&
        p.x >= node.x - tol &&
        p.x <= node.x + node.width + tol;
    if (onTop)
        return 'top';
    const onBottom = Math.abs(p.y - (node.y + node.height)) <= tol &&
        p.x >= node.x - tol &&
        p.x <= node.x + node.width + tol;
    if (onBottom)
        return 'bottom';
    const onLeft = Math.abs(p.x - node.x) <= tol &&
        p.y >= node.y - tol &&
        p.y <= node.y + node.height + tol;
    if (onLeft)
        return 'left';
    const onRight = Math.abs(p.x - (node.x + node.width)) <= tol &&
        p.y >= node.y - tol &&
        p.y <= node.y + node.height + tol;
    if (onRight)
        return 'right';
...
export function layoutTopology(nodes, edges, opts) {
    ...
    const appIds = uniqueSorted(visible
    ...
    const reachFamilyNodes = visible.filter((n) => resolveAppGroup(n) === 'reach');
    ...
    const reachChildRowW = 3 * NODE_W + Math.max(0, 2) * COL_GAP;
    let reachSectionW = 0;
    ...
    const webApiSectionW = webApiCount > 0
        ? webApiCount * NODE_W + Math.max(0, webApiCount - 1) * COL_GAP
        : 0;
    const webApiOriginX = MARGIN + (reachSectionW > 0 ? reachSectionW + APP_FAMILY_GAP : 0);
    const k8sNodeIds = uniqueSorted(visible.filter((n) => n.kind === 'k8s-node').map((n) => n.id));
    const k8sNodeLaneById = new Map(k8sNodeIds.map((id, i) => [id, i]));
    const k8sSvcIds = uniqueSorted(visible.filter((n) => n.kind === 'k8s-service' || n.kind === 'k8s-workload').map((n) => n.id));
    // Services sit to the right of node columns.
    const k8sSvcLaneById = new Map(k8sSvcIds.map((id, i) => [id, k8sNodeIds.length + i]));
    const podsByParent = new Map();
    for (const n of visible) {
        if (n.kind !== 'k8s-pod' || !n.parent)
            continue;
    ...
    const enriched = visible.map((n) => {
        ...
        if (n.kind === 'k8s-node') {
            ...
            return { node: n, band: 'ao', rank: 5, lane, order: lane };
        }
    ...
    enriched.sort((a, b) => {
        const bandOrder = { application: 0, reach: 1, ao: 2 };
    ...
    const appRowW = (reachSectionW > 0 ? reachSectionW : 0) +
    ...
    const k8sExpandRowW = k8sNodeIds.length + k8sSvcIds.length > 0
        ? Math.max(EXPAND_PANEL_W, k8sNodeIds.length * NODE_W +
            Math.max(0, k8sNodeIds.length - 1) * COL_GAP, k8sSvcIds.length * NODE_W +
            Math.max(0, k8sSvcIds.length - 1) * COL_GAP)
        : 0;
    const canvasContentW = Math.max(MAX_LANES * colWidth + ROUTE_MARGIN, appRowW + ROUTE_MARGIN, k8sExpandRowW + ROUTE_MARGIN);
    ...
    const serviceEntries = enriched.filter((e) => e.node.kind === 'k8s-service' || e.node.kind === 'k8s-workload');
    const rows = new Map();
    ...
    const bandOrderList = ['application', 'reach', 'ao'];
    const positioned = [];
    const bands = [];
    let y = MARGIN;
    for (const band of bandOrderList) {
        ...
        for (const [, rowNodes] of bandRows) {
            ...
            for (const e of rowNodes) {
                ...
                const isExpandHeader = isAppHeader ||
                ...
                if (!isAppHeader && !isWebApi) {
                    ...
                    if (isK8sChild) {
                        ...
                        while (usedLanes.has(lane) && lane < MAX_LANES - 1)
                            lane += 1;
    ...
    if (serviceEntries.length) {
        ...
        if (!Number.isFinite(stackBottom)) {
            ...
            stackBottom = aoNodes.reduce((m, n) => Math.max(m, n.y + n.height), MARGIN);
        }
    ...
    // Grow the AO band (and following canvas y) to enclose nested pod stacks + labels.
    {
        ...
        if (ao) {
            ...
            const needed = maxBottom + BAND_PAD_Y + K8S_GROUP_LABEL_H - ao.y;
            if (needed > ao.height) {
    ...
    for (const e of edges) {
        ...
        if (String(e.protocol || '') === 'k8s')
            continue;
        const from = byId.get(e.from);
        const to = byId.get(e.to);
        if (!from || !to)
            continue;
        const { fromSide, toSide } = chooseSides(from, to, String(e.kind || 'request'), positioned);
    ...
    const portOffsetFor = (node, side, key) => {
...
function boundsOfNodes(list, pad) {
    let minX = Infinity;
    let minY = Infinity;
    let maxX = -Infinity;
    let maxY = -Infinity;
...
function buildApplicationFamilyFrames(positioned) {
    ...
    const frames = [];
    for (const id of ['reach', 'web-api']) {
        const list = byFamily.get(id);
        if (!list?.length || !familyHasVisibleApps(id, list))
            continue;
...
function buildK8sGroupFrames(positioned) {
    const platform = positioned.find((n) => n.id === 'platform/k3s');
    if (!platform?.expandable)
        return [];
    const inventory = positioned.filter((n) => n.id === 'platform/k3s' ||
        n.kind === 'k8s-node' ||
        n.kind === 'k8s-pod' ||
        n.kind === 'k8s-service' ||
        n.kind === 'k8s-workload');
    ...
    const services = inventory.filter((n) => n.kind === 'k8s-service' || n.kind === 'k8s-workload');
    if (services.length) {
...
export function pathClosure(nodeId, edges) {
    const outs = new Map();
    const ins = new Map();
    for (const e of edges) {
        if (!outs.has(e.from))
    ...
    const walk = (start, map, forward) => {
        const stack = [start];
        while (stack.length) {
            const cur = stack.pop();
            for (const next of map.get(cur) || []) {
                const eid = edges.find((e) => forward ? e.from === cur && e.to === next : e.from === next && e.to === cur)?.id;
                if (eid)
                    edgeIds.add(eid);
                if (!nodes.has(next)) {
                    nodes.add(next);
...
export function slotForKind(kind) {
