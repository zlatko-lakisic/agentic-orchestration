...
export type TopologyNodeStatus = 'healthy' | 'degraded' | 'failed' | 'starting' | 'draining' | 'offline';
export type TopologyEdgeKind = 'request' | 'stream' | 'reverse-tunnel' | 'advertisement' | 'bypass';
export type TopologyNodeKind = 'app' | 'ui' | 'overlay-source' | 'local-tools' | 'openclaw' | 'ao-web' | 'ao-chat' | 'web-api-client' | 'session-bridge' | 'overlay-packer' | 'local-mcp-host' | 'speech-client' | 'mtls-enroller' | 'engine' | 'endpoint' | 'web-ui' | 'planner' | 'catalog' | 'model-backend' | 'model-runtime' | 'execution-backend' | 'worker' | 'mcp-sidecar' | 'platform' | 'storage' | 'k8s-workload' | 'k8s-node' | 'k8s-pod' | 'k8s-service' | string;
...
export interface TopologyNodeDetail {
    ...
    members?: {
        ...
        note?: string;
        pods?: TopologyNode['k8sResource'] extends {
            pods?: infer P;
        } ? P : unknown;
    } | null;
    generatedAt?: string;
