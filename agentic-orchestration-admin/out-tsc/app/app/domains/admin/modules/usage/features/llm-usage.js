...
function pieFromRows(rows, maxSlices = 7) {
    const sorted = [...(rows || [])]
        .map((r) => ({
    ...
    const series = top.map((r) => r.total);
    if (rest.length) {
        labels.push('Other');
 ...
 */
let LlmUsagePage = class LlmUsagePage {
    live = inject(AoLiveWs);
    clock = inject(AoClock);
    llmCols = ['key', 'calls', 'total'];
    txCols = ['when', 'app', 'model', 'tokens', 'status'];
    feedSeconds = signal(2);
    spendRanges = SPEND_RANGES;
    /** Rolling spend window (default past 6 hours). */
    spendRange = signal('6h');
    /** Per-panel Diagram / Table preference (defaults to diagram). */
    panelViews = signal({});
    txDataSource = new MatTableDataSource([]);
    data = computed(() => this.live.feeds()['llm_usage'] || null);
    error = computed(() => {
        const e = this.live.feedErrors()['llm_usage'] || this.live.feedErrors()['_'];
        return e || null;
    });
    updatedAgo = computed(() => {
    ...
    rangeLabel = computed(() => {
        const id = (this.data()?.spend?.window || this.spendRange());
    ...
    spendTooltip = computed(() => ({
        shared: true,
        x: {
            format: (this.data()?.spend?.granularity || '1d') === '1d'
                ? 'MMM dd'
                : 'MMM dd HH:mm',
        },
    }));
    ...
    transactions = computed(() => (this.data()?.recent || []).slice(0, 12).map((r) => mapTx(r)));
    recentSummary = computed(() => {
    ...
    budgetBars = computed(() => {
        ...
        const apiCalls = (this.data()?.api?.byAppId || []).reduce((n, r) => n + (r.calls || 0), 0);
    ...
    topApps = computed(() => this.data()?.llm?.byAppId || []);
    apiRequestCount = computed(() => (this.data()?.api?.byAppId || []).reduce((n, r) => n + (r.calls || 0), 0));
    compositionPie = computed(() => {
        const g = this.data()?.llm?.grandTotal;
    ...
    clientAppCount = computed(() => (this.data()?.llm?.byAppId || []).filter((r) => String(r.key || '').trim() && r.key !== '(unknown)').length);
    llmBlocks = computed(() => {
        const d = this.data();
        if (!d)
            return [];
    ...
    spendChart = {
        ...
        yaxis: {
            labels: {
                formatter: (v) => Math.abs(v) >= 1000 ? `${(v / 1000).toFixed(1)}k` : String(Math.round(v)),
            },
        },
    };
    ...
    setPanelView(id, value) {
        const mode = value === 'table' ? 'table' : 'diagram';
...
function windowCaption(t, hours) {
