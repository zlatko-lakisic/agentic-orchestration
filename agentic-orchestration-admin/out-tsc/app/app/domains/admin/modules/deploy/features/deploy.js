...
let DeployPage = class DeployPage {
    ...
    showDrift = signal(false);
    configuredCount = computed(() => this.config.entries().filter((e) => e.set).length);
    drift = computed(() => this.config
        .entries()
        .filter((e) => e.set &&
        !e.secret &&
        e.default != null &&
