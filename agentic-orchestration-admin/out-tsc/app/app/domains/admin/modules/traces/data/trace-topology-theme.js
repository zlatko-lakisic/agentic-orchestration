import { themeForKind } from '@/app/domains/admin/modules/topology/data/topology.theme';
/** Fuse uses `scheme-dark` / `scheme-light` on <html>, not Tailwind's `.dark`. */
export function isAoDarkScheme() {
    if (typeof document === 'undefined')
        return true;
    const root = document.documentElement;
    return (root.classList.contains('scheme-dark') ||
...
export function topologyKindForTraceActor(actor) {
    ...
    if (id === 'client' || id.startsWith('client'))
        return 'ui';
    if (id === 'planner' || id.includes('planner'))
        return 'planner';
    if (id === 'engine' || id.includes('engine'))
        return 'engine';
    if (id === 'orchestrator' || id.includes('orchestrator'))
        return 'execution-backend';
    if (id === 'mcp' || id.startsWith('mcp'))
        return 'mcp-sidecar';
    if (id === 'skills' || id.startsWith('skills'))
        return 'catalog';
    if (id.startsWith('agent:') || id.startsWith('agent_')) {
        return id.includes('ollama') ? 'model-runtime' : 'catalog';
    }
    if (id.includes('ollama') || id.includes('model'))
        return 'model-runtime';
    if (id.includes('inprocess') || id.includes('worker'))
        return 'worker';
    return 'platform';
}
export function themeForTraceActor(actor) {
    return themeForKind(topologyKindForTraceActor(actor));
}
/** Short display label for Mermaid participant boxes (avoid cut-off). */
export function shortTraceActorLabel(actor) {
 ...
 */
export async function applyTopologyStylesToMermaidSvg(svg, loadIcon) {
    ...
    const actorRects = Array.from(svg.querySelectorAll('g[class*="actor"] > rect, rect.actor, .actor-man rect, g.actor rect, g.participant > rect')).filter((r) => Number(r.getAttribute('width') || 0) > 20);
    if (!actorRects.length)
        return;
    const topY = Math.min(...actorRects.map((r) => Number(r.getAttribute('y') || 0)));
    const topRects = actorRects.filter((r) => Math.abs(Number(r.getAttribute('y') || 0) - topY) < 3);
    ...
    for (let i = 0; i < topRects.length; i++) {
            ...
            if (loadIcon) {
                try {
                    ...
                    if (iconSvg) {
                        ...
                        const cloned = iconSvg.cloneNode(true);
        ...
        g?.querySelectorAll('text').forEach((t) => {
            if (t.getAttribute('data-ao-topo') != null)
                return;
        ...
        svg.querySelectorAll('line.actor-line, .actor-line, line').forEach((line) => {
            ...
            if (vertical && Math.abs(x1 - cx) < 12) {
                ...
                line.style.setProperty('stroke', accent, 'important');
            }
        });
    }
    svg
        .querySelectorAll('path.messageLine0, path.messageLine1, line.messageLine0, line.messageLine1, .messageLine0, .messageLine1')
        .forEach((el) => {
        el.setAttribute('stroke', arrowAccent);
        el.style.setProperty('stroke', arrowAccent, 'important');
    });
}
...
function truncateToWidth(el, full, maxPx) {
    const raw = String(full || '').trim();
    if (!raw)
        return '';
    el.textContent = raw;
    try {
        if (el.getBBox().width <= maxPx)
            return raw;
    }
    catch {
