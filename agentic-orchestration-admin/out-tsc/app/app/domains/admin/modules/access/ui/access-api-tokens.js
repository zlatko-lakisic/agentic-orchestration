...
let AccessApiTokens = class AccessApiTokens {
    api = inject(AoApi);
    ...
    tokens = signal([]);
    error = signal(null);
    revokingId = signal(null);
    usageOpen = signal(false);
    usageToken = signal(null);
    usageRows = signal([]);
    usageError = signal(null);
    constructor() {
        effect(() => {
            ...
            const err = this.live.feedErrors()['access'] || this.live.feedErrors()['_'];
            if (err)
                this.error.set(err);
            else
                this.error.set(null);
    ...
    openMint() {
        ...
        const preferChatUi = !preferWebUi && !this.tokens().some((t) => t.assignedToChat);
