...
let AccessMtlsClients = class AccessMtlsClients {
    api = inject(AoApi);
    dialog = inject(MatDialog);
    columns = ['subject', 'serial', 'status', 'issued', 'actions'];
    clients = signal([]);
    error = signal(null);
    busyKey = signal(null);
    ngOnInit() {
    ...
    revokeByCn() {
        ...
        if (!subject?.trim())
            return;
        if (!confirm(`Revoke all mTLS certs for CN "${subject.trim()}"?`))
            return;
        this.api
            .revokeMtlsClient({
