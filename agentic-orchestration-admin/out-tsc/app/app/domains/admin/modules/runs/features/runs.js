...
let RunsPage = class RunsPage {
    ...
    error = signal(null);
    scopeNote = signal(null);
    detail = signal(null);
    copyFlash = signal(false);
    columns = ['id', 'scope', 'outcome', 'updated', 'mode', 'steps'];
    ...
    constructor() {
        effect(() => {
            ...
            if (err)
                this.error.set(err);
            const snap = this.live.feeds()['runs'];
            if (!snap)
                return;
            this.error.set(null);
    ...
    correlationId(d) {
        return (d.lastRunId || (d.scope === 'run_store' ? d.id : null) || null);
    }
    open(row) {
