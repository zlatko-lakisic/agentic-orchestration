...
let ControlPage = class ControlPage {
    ...
    flash = signal(null);
    busyId = signal(null);
    apps = computed(() => targetsInGroup(this.status()?.targets, 'apps'));
    stack = computed(() => targetsInGroup(this.status()?.targets, 'stack')[0] || null);
    host = computed(() => targetsInGroup(this.status()?.targets, 'host')[0] || null);
    hostArmed = computed(() => Boolean(this.status()?.hostControl?.armed));
    ngOnInit() {
    ...
    restart(target) {
        if (!target.available || this.busyId())
            return;
        ...
        this.dialog
            ...
            this.api
                ...
                const disconnect = r.data.disconnectLikely
                    ...
                    : '';
                this.flash.set(`Requested ${target.label}.${disconnect}`.trim());
                this.reload();
            });
        });
    }
