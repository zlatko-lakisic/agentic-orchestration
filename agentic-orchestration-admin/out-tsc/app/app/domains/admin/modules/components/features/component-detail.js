...
let ComponentDetailPage = class ComponentDetailPage {
    ...
    kind = computed(() => this.id());
    status = computed(() => String(this.topo()?.status || 'info'));
    fact = computed(() => this.topo()?.fact || '');
    endpoint = computed(() => {
        const t = this.topo();
        if (t?.urlHint)
            return t.urlHint;
        ...
        if (this.id() === 'openclaw')
            return 'http://<host>:30487/api/v1/orchestrate';
        return null;
    });
    notes = computed(() => {
        const map = {
    ...
    settingsGroups() {
        ...
        if (id === 'execution')
            return ['execution'];
        if (id === 'engine')
            return ['engine', 'security'];
        if (id === 'ollama')
            return ['models'];
        if (id === 'web')
            return ['deployments', 'security'];
    ...
    testConnection() {
        ...
        if (this.id() === 'web') {
            this.api.ping().subscribe((r) => {
                this.probeResult.set(r.ok ? `OK — ${r.data.service} pid ${r.data.pid}` : r.message);
            });
            return;
        }
        if (this.id() === 'engine') {
            this.api.topology().subscribe((r) => {
                ...
                this.probeResult.set(eng?.status === 'healthy'
                    ? `OK — ${eng.fact}`
        ...
        if (this.id() === 'ollama') {
            this.api.topology().subscribe((r) => {
                const o = r.ok
                    ...
                    : null;
                this.probeResult.set(o ? `${o.status}: ${o.fact}` : r.ok ? 'No ollama component' : r.message);
            });
        }
    }
};
ComponentDetailPage = __decorate([
    Component({
