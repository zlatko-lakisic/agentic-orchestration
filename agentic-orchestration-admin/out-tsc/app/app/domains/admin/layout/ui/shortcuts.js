...
let Shortcuts = class Shortcuts {
    ...
    // State
    search = signal('');
    bookmarks = signal(this.restoreBookmarks());
    items = NAVIGATION.flatMap((group) => group.children ?? []).filter((item) => !!item.route);
    bookmarkedItems = computed(() => this.bookmarks()
        .map((id) => this.items.find((item) => item.id === id))
        .filter((item) => !!item));
    visibleBookmarks = computed(() => this.bookmarkedItems().slice(0, 5));
    overflowCount = computed(() => this.bookmarkedItems().length - this.visibleBookmarks().length);
    isMobile = computed(() => this.media.match(`(max-width: 639px)`)());
    badgeCount = computed(() => this.isMobile() ? this.bookmarkedItems().length : this.overflowCount());
    badgeLabel = computed(() => this.isMobile() ? `${this.badgeCount()}` : `+${this.badgeCount()}`);
    ...
    toggleBookmark(id) {
        this.bookmarks.update((bookmarks) => bookmarks.includes(id)
