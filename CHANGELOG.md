...
- **API authentication always required** — `POST /api/v1/orchestrate`, `/v1/chat/completions`, and `/v1/responses` never accept anonymous calls. Minted Bearer tokens (or env shared-secret fallback) are mandatory. Admin / operator HTTP APIs require the reserved **`ao-web`** Web UI token once assigned; until then only Access bootstrap routes (posture, token mint/list, `GET /api/v1/admin/web-auth`) stay open so operators can mint it. Minting `ao-web` (or `assignToWeb`) auto-assigns the secret to the Admin SPA.
