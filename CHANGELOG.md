...
- **Reach / Comstar `No module named 'fastapi'` on Jetson** — VS Code runs hit warm-pool workers executing CrewAI steps with MCP tools; litellm's MCP handler imports `fastapi`, but routine deploys only patched warm-pool volume mounts and never re-applied `deploy/k8s/warm-pool.yaml`, so long-lived worker pods kept the old container entrypoint without the startup `pip install fastapi` bootstrap. Added `jetson-sync-warm-pool.sh` (apply manifest + rollout restart + import probe) and wired it into `jetson-deploy.sh`; warm-pool startup now uses `python -m pip` explicitly.
