...
- **Overlay agents inherited the stock MCP catalog** — empty `allowedMcpProviderIds` meant unrestricted. Overlay sessions now plan against `client.*` (plus any explicit stock pin). `client.*` tasks drop stock tools such as `search_tavily` / host `filesystem_local` unless pinned. COMSTAR workspace tunnel stays `client.filesystem_local`. Planner keyword prune no longer strips packed `client.*` MCPs when the goal has no catalog tags (that dropped the filesystem tunnel on “read hello.txt”).
