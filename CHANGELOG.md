...
- **Reach / Comstar `No module named 'fastapi'` on Jetson** — VS Code runs hit warm-pool workers executing CrewAI steps with MCP tools; litellm's MCP handler imports `fastapi`, but routine deploys only patched warm-pool volume mounts and never re-applied the fastapi bootstrap entrypoint. Added `jetson-sync-warm-pool.sh` (patch-only bootstrap + re-apply Jetson volume patches + rollout restart + import probe) and wired it into `jetson-deploy.sh`; warm-pool startup now uses `python -m pip` explicitly. The first version used a full `kubectl apply` of `warm-pool.yaml`, which wiped Jetson hotfix mounts and left the rollout stuck at `1 out of 2 new replicas`; fixed to patch command/args only via `warm-pool-fastapi-bootstrap-patch.yaml`.
