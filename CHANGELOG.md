...
- **Topology health glyphs** — each node status has its own mark: green check (healthy), yellow caution triangle (degraded), red X (failed), spinner (starting), down-arrow (draining), question (unknown), slashed circle (offline). Legend and table use the same icons.
