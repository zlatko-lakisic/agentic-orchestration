...
- **Edge default: in-cluster Ollama** — `config/env.jetson` sets `AGENTIC_OLLAMA_MODE=managed_k8s` and `OLLAMA_API_BASE=http://agentic-ollama:11434`. Migrate host systemd installs with `scripts/jetson-migrate-ollama-to-k8s.sh` (copies models into `var/ollama-models`, enables the Deployment, removes host `ollama.service`).
