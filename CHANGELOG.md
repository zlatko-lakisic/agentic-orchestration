...
- **Reach run heartbeats** — while a run executes, the engine WebSocket repeats the current phase as a `status` frame with the elapsed time every `AGENTIC_SERVE_HEARTBEAT_SECONDS` (default 10, `0` disables), tagged `heartbeat: true` with `elapsedMs`. Silent stretches (planning, a slow model call, warm-pool dispatch) emit no progress lines, so clients previously saw a dead-looking socket and cut runs off on their own deadlines.
- **Background activity (overlay / Ollama pull / MCP / planner)** — slow work before a `run_id` exists streams as Reach `status` (`processing: true`, `phase: preparing`, named `message` such as `Downloading qwen3.6:27b — 84%`). Host-metrics carries `backgroundActivity`; Reach sessions list a `prepare` object. Chat and Admin host-utilization show the same banner with a percent bar and **Cancel** on `model_pull`. `POST /api/v1/admin/background-activity/cancel` (and WS `background_activity_cancel`) abort the live `POST /api/pull` or `ollama pull` subprocess; Comstar Stop, overlay clear, and WS disconnect do the same. Partial blobs are left for resume; overlay is not acked until ensure finishes.
- **Reach multimodal turns (images on `chat` / `direct_agent`)** — WebSocket messages accept optional ordered `images: [{mimeType, dataBase64, name?}]` (`image/jpeg|png|webp|gif`). A non-empty list skips the planner and CrewAI and runs a single vision completion via LiteLLM (`orchestration/reach_multimodal.py`), streaming the plain-text answer over the usual `chunk` / `status` / `run_end` frames. Omitting the field keeps the existing text-only behavior. Model resolution: client `[model=…]` prefix → `AGENTIC_REACH_VISION_MODEL` → overlay agent `model` → `AGENTIC_MEDIA_VISION_MODEL` / `AGENTIC_VIDEO_VISION_MODEL` → `openai/gpt-4o-mini` when `OPENAI_API_KEY` is set. Text-only candidates are skipped at every level and a request with no vision model left fails with `vision_unavailable` rather than answering a camera prompt it never saw. Limits (`AGENTIC_REACH_MAX_IMAGES` 16, `AGENTIC_REACH_MAX_IMAGE_BYTES` 4 MiB, `AGENTIC_REACH_MAX_IMAGES_TOTAL_BYTES` 20 MiB) reject with `invalid_images` / `payload_too_large` before a run starts.
- **Jetson Ollama catalog expansion** — agent YAMLs for Gemma 4 (e2b/e4b/12b/26b), Qwen3.5 (4b/9b/27b), Llama 3.1 8B, muse-glimmer, Nemotron 3.5 Lightning, Qwen2.5 7B, Mistral NeMo, DeepSeek-R1 14B, GLM-4, Moondream, LFM2 24B, Granite 4.1 8B, Phi-4, Granite Code, LFM2.5 Thinking, and OLMo 2; batch pull via `scripts/jetson-pull-ollama-models.sh` into `/mnt/nvme/ollama/models`.
...
- **COMSTAR / CrewAI empty LLM on thinking models** — Qwen3.6 and similar can bill completion tokens while leaving `message.content` empty (`thinking` / `reasoning_content` / `<think>`). LiteLLM completions now copy that text into `content` before CrewAI’s `Invalid response from LLM call - None or empty` check; still-empty replies raise `model returned no text (context or thinking-only)`. Ollama agents retry once with tools stripped.
- **Overlay agents inherited the stock MCP catalog** — empty `allowedMcpProviderIds` meant unrestricted. Overlay sessions now plan against `client.*` (plus any explicit stock pin). `client.*` tasks drop stock tools such as `search_tavily` / host `filesystem_local` unless pinned. COMSTAR workspace tunnel stays `client.filesystem_local`. Planner keyword prune no longer strips packed `client.*` MCPs; empty planned MCP lists are filled from the overlay agent YAML. Steps that use the session MCP tunnel run in-process on the engine (the tunnel loopback is `localhost` and is unreachable from warm-pool workers). CrewAI prefixes that loopback URL into tool names until they hash past 64 characters; native MCP tools are renamed to the MCP original (`read_file`, `list_directory`, …) and Ollama overlay agents also get short CrewAI `read_file` / `list_directory` shims that JSON-RPC to the tunnel (same pattern as `fetch`). When the user names a file, execute_step reads it over the tunnel first and then summarizes, because qwen-class models often never invoke CrewAI tools. If the tunnel text already shows `return a - b` in `add()`, the engine names that subtraction bug without a thinking-model summarize (qwen3.6:27b otherwise spent the 20-minute live budget on three lines). Echo/truncation asks return the MCP text directly.
- **MCP tool results could refill the 16k Ollama context** — CrewAI tool `_run` output and long Reach user transcripts are clipped (`AGENTIC_TOOL_RESULT_CHARS` / `AGENTIC_REACH_PROMPT_CHARS`). Further tool rounds stop when prompt tokens are within 1024 of `AGENTIC_OLLAMA_CONTEXT_LENGTH` (still **16384** on Jetson and Ada; do not raise it — granite-code KV-cache OOM).
- **CrewAI identity-fill for Continue {workspaceName} chat text** — CrewAI interpolate_only treated brace placeholders in user chat (e.g. Continue {workspaceName}) as missing task-template variables and aborted kickoff. Kickoff now identity-fills unmatched {identifier} names in inputs so the LLM still sees the literal braces. New crewai_template.py is mounted via Jetson/Ada hostPath (ConfigMap hotfix is at 1 MiB).

- **Reach / Comstar `No module named 'fastapi'` on Jetson** — VS Code runs hit warm-pool workers executing CrewAI steps with MCP tools; litellm's MCP handler imports `fastapi`, but routine deploys only patched warm-pool volume mounts and never re-applied the fastapi bootstrap entrypoint. Added `jetson-sync-warm-pool.sh` (patch-only bootstrap + re-apply Jetson volume patches + rollout restart + import probe) and wired it into `jetson-deploy.sh`; warm-pool startup now uses `python -m pip` explicitly. The first version used a full `kubectl apply` of `warm-pool.yaml`, which wiped Jetson hotfix mounts and left the rollout stuck at `1 out of 2 new replicas`; fixed to patch command/args only via `warm-pool-fastapi-bootstrap-patch.yaml`. **Ada / single-node:** warm-pool `RollingUpdate` with `replicas: 2` could exceed `progressDeadlineSeconds` when old and new pods overlap — `jetson-warm-pool-rollout.sh` applies **Recreate** strategy and stale ReplicaSet cleanup (same pattern as coordinator). **Ada / Jetson warm-pool Python:** workers always use the *image* interpreter (`AGENTIC_PYTHON=python`), never the host `.venv` (Ada `/usr/bin/python3.12` vs image `/usr/local/bin/python`). Missing `fastapi` is pip-installed into the image; host venv `site-packages` is a fallback only if pip fails. Host `.venv` is mounted only when it already exists. Same bootstrap on Jetson (ARM) and Ada (x86).
- **AO footprint panel styling** — the share dial's centre printed the literal string `AO share` where the number belongs (`total.label` and `total.formatter` were swapped) so no percentage ever rendered; it now shows the RAM share of host. The breakdown chart left its category axis unstyled, which meant Apex's near-black default fill on the dark dashboard and labels clipped at 160px (`llama3.2:3b (ollama_l…`) — the axis now uses the same `--mat-sys-on-surface` colour as the CPU/GPU charts with room for long rows, and model rows list one agent plus a `+N` count. Also dropped the duplicate RAM/VRAM legend under the dial (the stat blocks carry colour dots instead), shrank the oversized dial, and moved the blue/green palette into shared constants so the panel cannot drift from the charts above. The dial now sits to the right of the AO RAM / AO VRAM figures, and its centre labels use explicit light/dark fills so Apex no longer paints them near-black.
- **Ollama resource-broker rollout took `:11434` down on Jetson and Ada** — the sidecar shipped with image `agentic-orchestrator-coordinator:local`, a dev-only tag that exists on no edge node, so the pod sat in `ImagePullBackOff`, `agentic-ollama` never became Ready, its Service had no endpoints, and every Ollama-backed agent lost `OLLAMA_API_BASE=http://agentic-ollama:11434`. `jetson-enable-ollama.sh` now resolves the sidecar image from the image that cluster's own `agentic-coordinator` runs (`AGENTIC_OLLAMA_BROKER_IMAGE` still overrides), mounts the git checkout's `orchestration/` at `/app/tool/orchestration` because the broker modules are newer than any pinned image, and installs `fastapi` at startup when the image predates it (`requirements-serve.txt` only). Added `AGENTIC_OLLAMA_RESOURCE_SHARING=0` to serve plain Ollama on `:11434` with no sidecar, `AGENTIC_OLLAMA_DRY_RUN=1` to print the rendered manifest without applying, and pod/container detail on a failed rollout instead of a bare `timed out waiting for the condition`.
- **Ollama daemon restarted every few minutes under the broker (image mode)** — the daemon binds `127.0.0.1:11435`, but its readiness/liveness probes had no `host`, so kubelet dialed the pod IP, got connection refused, and liveness killed the container in a loop. Probes now target `127.0.0.1`.
- **Models hostPath dropped sibling volumes** — with `AGENTIC_OLLAMA_MODELS_HOSTPATH` set, the script rebuilt the pod's `volumes:` list from scratch, discarding the NVIDIA device and orchestration-source volumes that containers still mount. It now inserts the models volume after `ollama-home`.
- **`/v1/chat/completions` answered image prompts with text-only models** — an OpenAI-shaped body carrying `image_url` / `input_image` parts is now rejected with `model_not_multimodal` when the requested model cannot accept images, and the Ollama 404 fallback (`AGENTIC_OLLAMA_PROXY_FALLBACK_MODEL`, typically a text instruct model) is skipped on multimodal requests instead of silently answering from the prompt text.
- **Session overlay agents pulled OpenAI models from Ollama** — a client entry declaring `type: ollama` with `model: gpt-4o-mini` made the engine retry `ollama pull gpt-4o-mini` forever. Overlay registration now coerces such entries to `type: openai`, and they are excluded from the Ollama ensure queue.
- **Warm-pool provider recovery returns stale failure** — on retry, `dispatch_step_via_warm_pool` reuses the same `step_id` and `wait_for_warm_pool_result` accepted the previous attempt's `result.json` immediately (~0.3 ms), discarding a successful re-execution. Now invalidates the prior result before enqueue and ignores results older than the enqueue timestamp.
- **Jetson granite-code OOM (128k KV cache)** — `granite-code` (Granite 3B Code Instruct 128k) sized a ~39 GiB KV cache and crashed the Ollama runner (`cudaMalloc failed` / `llama runner process has terminated`). Daemon now sets `OLLAMA_CONTEXT_LENGTH=16384` (overridable via `AGENTIC_OLLAMA_CONTEXT_LENGTH`); Jetson agent YAMLs can also set per-agent `num_ctx`, forwarded into the CrewAI/LiteLLM call. Corrected `ollama_lfm2_5_thinking` `min_vram_gb` from 14 → 4 (model is ~731 MB).
- **Chat agent dropdown ignored the active catalog** — the web "Select agent provider" list was hardcoded to the bundled `config/agent_providers`, so on Jetson it advertised desktop-only agents (e.g. `ollama_granite_code`, `ollama_lfm2_5_thinking`, `ollama_olmo2`) that the orchestrator's edge catalog never planned against; selecting them was silently dropped. `server.mjs` now resolves the dropdown catalog from `AGENTIC_AGENT_PROVIDERS_CATALOG` (relative to the tool root, matching `main.py`), so the UI matches what the planner uses. Added Jetson-tuned variants of those three agents (`selfcontained: false`, in-cluster host) plus their tags in `jetson-pull-ollama-models.sh`.
- **Dynamic planner hard-fails on stale agent chips** — when chat `selectedAgentProviderIds` name agents missing from the edge catalog (or filtered by hardware / unpulled models), the planner now keeps only survivors; if none survive it ignores the selection and uses the full filtered catalog instead of raising `No agent providers left after applying explicit agent selection`.
- **Planner Ollama ensure noise under managed_k8s** — `ensure_planner_runtime_if_ollama` no longer tries the host `ollama` binary at `127.0.0.1` when ownership is `external` / `managed_k8s`; it skips ensure and (when spawn is allowed) uses `OLLAMA_API_BASE` via `litellm_api_base_for_ollama`. Jetson mounts `runtime_bootstrap.py` / `ollama_catalog_filter.py` / `host_metrics.py` from the git hostPath (orchestration ConfigMap is at the 1 MiB limit).
...
- **Chat agent picker is a searchable list** — the Agents `<select>` is now a combobox: free-text search across agent id, backend, role, and planner hint (all tokens must match), plus an **All** button that expands the entire active catalog grouped by backend. Selected agents move to chips and are excluded from the list; arrow keys + Enter add the highlighted entry, Escape collapses. The list expands in flow because the settings form sits in a scrollable rail/sheet that would clip an overlay.
...
- **Engine probes over TLS** — `agentic-engine` readiness/liveness use `httpGet` with `scheme: HTTPS` so probes work when the daemon serves TLS (plain HTTP probes caused EOF / restarts).
...
- **Kubernetes expand: nodes, pods, services, network paths** — expanding Kubernetes shows labeled containment frames: outer **Kubernetes**, per-**node** frames with pods nested tightly underneath, and a **Services** group. Hierarchy uses frames (not parent→child arrows); Service→Pod endpoint edges remain. Coordinator RBAC gains `services`/`endpoints` list plus optional ClusterRole for Nodes.
...
- **English-default reply locale / CJK leak guard** — Qwen-style mid-answer flips into Chinese (or full-CJK follow-ups) are stripped in `sanitize_user_facing_prose` / web `text-normalize` when `AGENTIC_REPLY_LOCALE=en` (default). Harness `forbids_regex` on general/voice/research/write/reason; impartial QA merges the same assertion (plus `config/impartial_qa_assertions.json` on edge). Qwen provider prompts and delivery-format suffix require matching the user language.
- **Voice JSON → prose** — when a Final Answer is a JSON object/array (common on COMSTAR / HA tool turns), `sanitize_user_facing_prose` unwraps speakable fields (`spoken`, `answer`, `text`, …) and blanks opaque machine dumps so TTS does not read braces aloud.
