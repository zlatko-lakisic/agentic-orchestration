...
- **Agent provider choice:** For each step, pick the **single best** `agent_provider_id`. Judge from the user's task and each entry's `planner_hint`, `role`, `goal`, `model`, and `type` (`ollama` = local host, `openai` = OpenAI-compatible cloud API, `anthropic` = Anthropic Claude API, `huggingface` = Hugging Face Hub inference, `deterministic` = fixed Python entrypoint with no LLM). No default bias toward local vs cloud.
- **Deterministic agents:** Prefer `type: deterministic` for scoring, rule evaluation, calibrated probabilities, fixed transforms, or any step whose `planner_hint` describes a pure function over structured inputs. Never assign them to open-ended prose, creative writing, or multi-hop research — those need an LLM type.
