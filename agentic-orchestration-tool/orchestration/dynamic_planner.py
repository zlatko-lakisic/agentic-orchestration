...
- **Attached files:** If the user message includes `## Attached files`, use the listed categories (tabular, code, image, document, media, …), MIME types, and absolute paths to route work—e.g. data-heavy files to analysis/integration agents, code to engineering agents, images (including JPEG frames extracted from videos) to multimodal agents when available; read any **Automated vision synopsis** text as a first-pass scene summary.
