...
def _stock_ids(*, catalog_root: Any | None) -> set[str]:
    ...
    try:
        ...
        # Temporarily clear overlay context so merge hooks do not recurse.
...
def register_overlay(
    *,
    ...
    validated_agents = [
        _validate_agent_entry(a, index=i) if isinstance(a, dict) else (_ for _ in ()).throw(
            SessionOverlayError(f"agents[{i}] must be a mapping")
        )
        for i, a in enumerate(raw_agents)
    ]
    # Rewrite the awkward throw hack — use a normal loop:
    validated_agents = []
    for i, a in enumerate(raw_agents):
