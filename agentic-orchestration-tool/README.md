...
**Impartial QA gate (v1, on by default, advisory)**: the finalization points above run one unified check — harness assertions, the judge score (`evaluate_run_quality`, optional rubric appended to the goal like harness L3), and the faithfulness review combined into a single pass/fail report under `=== Impartial QA (unified gate) ===`, persisted to `__orchestrator_sessions__/impartial_qa/`. It also runs after `--society` (scoring the panel's final recommendation) and after a routed static workflow. Nothing is re-executed. Two independent switches: `AGENTIC_IMPARTIAL_QA=0` disables the gate, and `AGENTIC_IMPARTIAL_QA_FAIL=1` promotes it to a **hard gate** that exits `1` after artifacts are saved — until then a low score is reported, not enforced. When no reviewer can run and no assertions are configured (e.g. `AGENTIC_LEARNING_EVAL=0` + `AGENTIC_FINAL_QA=0` on the edge device), the gate reports itself skipped instead of failing. See `.env.example` for `AGENTIC_IMPARTIAL_QA_MIN_SCORE`, `_EVAL`, `_RUBRIC`, `_ASSERTIONS_FILE`, `_FAITHFULNESS_FAIL`; smoke via `scripts/smoke_impartial_qa.sh`.
