...
**Impartial QA / outcome scoring** — **Unification started (v1), on by default in advisory mode.** Three separate mechanisms grew up independently: the learning-loop eval (`AGENTIC_LEARNING_EVAL`), platform harness L3 capability scoring (`--harness-tier capability`), and user-harness rubric scoring (`--harness-dir`). A unified gate now scores the finished deliverable once — harness assertions, the judge score against an optional rubric, and the faithfulness review in a single pass/fail report — after `--dynamic` or `--dynamic-iterative` finalization, after `--society`, and after a routed static workflow. It re-executes nothing. It runs unless **`AGENTIC_IMPARTIAL_QA=0`**, and a failing report is *reported* rather than enforced until you set **`AGENTIC_IMPARTIAL_QA_FAIL=1`**, which turns it into a hard gate that fails the run before the output is treated as done. The harness CLI and learning loop are untouched; folding the harness paths themselves into the same gate is the remaining work.
