...
**Impartial QA / outcome scoring** — **Unification started (v1), still opt-in.** Three separate mechanisms grew up independently: the learning-loop eval (`AGENTIC_LEARNING_EVAL`), platform harness L3 capability scoring (`--harness-tier capability`), and user-harness rubric scoring (`--harness-dir`). A first unified gate now sits behind **`AGENTIC_IMPARTIAL_QA=1`**: after `--dynamic` or `--dynamic-iterative` finalization it scores the finished deliverable once — harness assertions, the judge score against an optional rubric, and the faithfulness review in a single pass/fail report — and can fail the run before the output is treated as done. It re-executes nothing and is off by default; the harness CLI and learning loop are untouched. Folding the harness paths themselves into the same gate is the remaining work.
