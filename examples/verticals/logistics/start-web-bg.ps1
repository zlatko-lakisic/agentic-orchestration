...
if ($Port -le 0) {
  $p = ($env:AGENTIC_WEB_PORT ?? "3851").Trim()
  try { $Port = [int]$p } catch { $Port = 3851 }
}
...
try {
  if (-not (Test-Path -Path "node_modules")) {
    Write-Host "[logistics-web-bg] node_modules missing -> npm install"
    npm install
  }
} finally {
  Pop-Location
...
if (Test-Path $pidFile) {
  try {
    ...
    Write-Host "[logistics-web-bg] already running (pid=$oldPid). Stop with: .\stop-web.ps1"
    exit 0
  } catch {
  }
}

Write-Host "[logistics-web-bg] starting detached server..."
Write-Host "[logistics-web-bg] log: $logFile"

$proc = Start-Process -FilePath "node" `
  -ArgumentList @("server.mjs", "--example", "logistics") `
...
Write-Host "[logistics-web-bg] started pid=$($proc.Id) (http://${Host}:$Port/)"
