...
if (-not (Test-Path $pidFile)) {
  Write-Host "[logistics-web] no pid file ($pidFile)."
  exit 0
}
...
try {
  ...
  Write-Host "[logistics-web] stopped pid=$serverPid"
} catch {
  Write-Host "[logistics-web] process not running (pid=$serverPid)"
}

Remove-Item -Force $pidFile -ErrorAction SilentlyContinue

if (($env:AGENTIC_WEB_KILL_PORT ?? "").Trim() -eq "1") {
  Write-Host "[logistics-web] AGENTIC_WEB_KILL_PORT=1 -> try stopping listeners on port $port (Get-NetTCPConnection / taskkill as needed on Windows)."
}
