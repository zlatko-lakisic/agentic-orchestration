---
layout: default
title: Agent provider catalog
permalink: /Agent-provider-catalog/
---
...
| `general_purpose` | When `true`, treated as a broad default (e.g. research); orchestration may omit these from the planner catalog when specialist entries align much better with the user goal lexically (`AGENTIC_DOMAIN_PROVIDER_*` in `.env.example`). |
...
## Ollama (`ollama_*`) — 84 files

- `ollama_agribot.yaml`
- `ollama_agrillama.yaml`
