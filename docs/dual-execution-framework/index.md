...
| Topic | This plan (dual framework) | [Kubernetes execution upgrade]({{ '/kubernetes-execution-upgrade/' | relative_url }}) |
|-------|---------------------------|----------------------------------|
...
Existing YAML fields (`id`, `type`, `role`, `goal`, `backstory`, `model`, `stdio` / `streamable_http` MCP blocks, `required_env`, `planner_hint`, etc.) remain authoritative. See [Agent provider catalog]({{ '/agent-catalog/' | relative_url }}), [MCP providers]({{ '/mcp-catalog/' | relative_url }}), [Workflows and router]({{ '/workflows/' | relative_url }}).
...
- [x] **G5** Wiki/catalog docs ([Agent provider catalog]({{ '/agent-catalog/' | relative_url }}), [MCP providers]({{ '/mcp-catalog/' | relative_url }})) stay single source of truth; backends do not get separate catalog pages.
- [x] **G6** New execution-backend behavior covered by [Testing and CI]({{ '/testing-and-ci/' | relative_url }}) markers (`backend_inprocess` + `backend_subprocess` shipped; `backend_kubernetes` deferred to F5).
...
Shared with [Kubernetes execution upgrade]({{ '/kubernetes-execution-upgrade/' | relative_url }}) — canonical JSON schemas live there; Python types mirror them.
...
Fields align with K8s plan [step spec JSON schema]({{ '/kubernetes-execution-upgrade/' | relative_url }}#step-spec-json-schema-draft-v01) for worker transport only.

### `StepResult`

One step outcome. Aligns with K8s plan [worker result contract]({{ '/kubernetes-execution-upgrade/' | relative_url }}#worker-result-contract-draft-v01).
...
**Maps to:** K8s plan Phase 3 — implementation details in [Kubernetes execution upgrade]({{ '/kubernetes-execution-upgrade/' | relative_url }}).
...
- Emit `(progress)` lines
- Invoke per-step `execution_fallback` / provider recovery — **deferred** (workflow-level retry in `main` today; see [Kubernetes execution upgrade]({{ '/kubernetes-execution-upgrade/' | relative_url }}#post-f4-plan-adjustments-2026-06))
...
- [x] **F0.2** Align field names with [step spec]({{ '/kubernetes-execution-upgrade/' | relative_url }}#step-spec-json-schema-draft-v01) and [result JSON]({{ '/kubernetes-execution-upgrade/' | relative_url }}#worker-result-contract-draft-v01) in K8s plan.
- [x] **F0.3** Add `execution_backend_from_env()` (returns `CrewAIExecutionBackend` by default; lazy-loads other backends).
- [x] **F0.4** Document `AGENTIC_EXECUTION_BACKEND` in [Configuration]({{ '/configuration/' | relative_url }}) and `.env.example`.
...
- [x] **F4.4** `AGENTIC_EXECUTION_BACKEND=subprocess` documented (`.env.example`, [Configuration]({{ '/configuration/' | relative_url }})).
...
| Phase | Tests |
|-------|--------|
| F0 | Type/import smoke | See [Testing and CI]({{ '/testing-and-ci/' | relative_url }}) |
...
|------|----------|-----------|
| 2025-06-26 | Dual plans: this page (framework seam) + [Kubernetes execution upgrade]({{ '/kubernetes-execution-upgrade/' | relative_url }}) (cluster) | Separate concerns; framework ships value without K8s |
...
**Next (K8s / F5):** See [Kubernetes execution upgrade]({{ '/kubernetes-execution-upgrade/' | relative_url }}#suggested-work-order-current--post-f4) — K0.6 → K2.3 worker image → K3.0 dispatch → K3.1–3.8 kind test → K4 sidecars.
...
- [x] [Configuration]({{ '/configuration/' | relative_url }}) — `AGENTIC_EXECUTION_BACKEND`, `AGENTIC_SUBPROCESS_WORKERS`.
- [x] [CLI reference]({{ '/cli-reference/' | relative_url }}) — `--execute-step` worker flag.
- [x] [Testing and CI]({{ '/testing-and-ci/' | relative_url }}) — F2 unit tests checklist.
- [ ] [Architecture]({{ '/architecture/' | relative_url }}) — execution backend diagram (partial module list updated).
- [x] Cross-check [Kubernetes execution upgrade]({{ '/kubernetes-execution-upgrade/' | relative_url }}) phase checkboxes after F4 (wiki synced 2026-06).
