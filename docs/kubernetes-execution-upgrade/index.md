...
| | [Dual execution framework]({{ '/dual-execution-framework/' | relative_url }}) | This page (K8s upgrade) |
|---|-------------------------------|-------------------------|
...
| **Shared YAML config** | Agent, MCP, and workflow YAML unchanged across backends — [Dual execution framework]({{ '/dual-execution-framework/' | relative_url }}#shared-configuration-yaml-unchanged). |
...
**Companion plan:** Pair with [Dual execution framework]({{ '/dual-execution-framework/' | relative_url }}) Phase **F0** (Python types must match schemas below).
...
**Companion plan:** Overlaps [Dual execution framework]({{ '/dual-execution-framework/' | relative_url }}) Phases **F2** (materializer, `StepCoordinator`) and **F1** (CrewAI backend extract). Coordinate so run store and step specs are not implemented twice.
...
- [x] **5.1** Warm pool: idle worker pods + coordinator dispatch (`deploy/k8s/warm-pool.yaml`, `kubernetes_warm_pool.py`).
- [x] **5.2** Centralized logging contract (`deploy/k8s/LOGGING.md`, `AGENTIC_LOG_FORMAT=json`).
...
The protocol, factory, and three backend classes are specified in [Dual execution framework]({{ '/dual-execution-framework/' | relative_url }}) — not duplicated here.
...
**No new YAML schema** — runtime policy only ([Dual execution framework]({{ '/dual-execution-framework/' | relative_url }}#shared-configuration-yaml-unchanged)).
...
| 2025-06-26 | Split plans: [Dual execution framework]({{ '/dual-execution-framework/' | relative_url }}) (code seam) + this page (K8s) | Framework F0–F4 before K8s Phase 3 |
...
**Framework plan ([Dual execution framework]({{ '/dual-execution-framework/' | relative_url }})):** F0 → F1 → F2 → F3 → F4 ✅
...
- [x] Update [Configuration]({{ '/configuration/' | relative_url }}) with K8s env vars — covered under Execution backend / `AGENTIC_K8S_*` / run-store sections (K3+ shipped).
